...
from AnonXMusic.info import (get_served_chats, get_served_users, del_served_chat, del_served_user, activecall, add_active_chat, add_served_call, add_active_video_chat)
from AnonXMusic.Data import Bots
from AnonXMusic.play import (logs, join_call)
from AnonXMusic.Data import (get_userbot, get_dev, get_call, get_group, get_channel)
import aiohttp
import asyncio
...
async def chats_func(client: Client, message: Message):
 ...
 if message.chat.id == dev or message.chat.username in OWNER:
    ...
    if count == 0:
      return await m.edit("**♪ لا يوجد جروبات  💎 .**")
    else:
      try:
    ...
    for served_chat in served_chats:
        if f"{served_chat}" in text:
         ...
         try:
            ...
            text += f"{count}:- {title} {served_chat}\n"
    if count == 0:
      return await m.edit("**♪ لا يوجد مستخدمين  💎 .**")
    else:
      try:
        await message.reply_text(text, disable_web_page_preview=True)
      except: 
...
@Client.on_message(filters.command("تغير الاسم الثاني", ""))
...
@Client.on_message(filters.command(["ازالة الصور"], ""))
